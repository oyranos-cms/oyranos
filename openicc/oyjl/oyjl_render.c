...
static int oyjlArgsRendererSelect_  (  oyjlUi_s          * ui )
{
  ...
  if( !R ||
      (R && R->option && strcasecmp(R->option,"render") != 0))
    R = oyjlOptions_GetOptionL( ui->opts, "render", 0 );
  ...
  else
  {
    if(arg[0])
    {
      ...
      if(!name)
      {
        oyjlMessage_p( oyjlMSG_INFO, 0, OYJL_DBG_FORMAT "\"-R|--render\" not supported: %s", OYJL_DBG_ARGS, oyjlTermColor(oyjlBOLD,arg) );
        return 1;
      }
