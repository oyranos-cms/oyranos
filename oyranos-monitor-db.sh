...
if [ "$1" = "-h" ]; then
  echo Usage:
  echo "  $0 [oyranos-monitor options like -d or -x and -y]"
  echo ""
  exit 0
fi

model=`oyranos-monitor $@ -f edid | xcmedid | grep model | grep -v model_id | sed 's/\"//g; s/model: //'`
short_name=`oyranos-monitor $@ -f edid | xcmedid | grep mnft | grep -v mnft_id | sed 's/\"//g; s/mnft: //'`

echo "local monitor: \"$short_name\" \"$model\""
