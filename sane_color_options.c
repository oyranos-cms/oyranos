...
#define BUFSIZE 1024
static unsigned int
print_int_vec( const SANE_Int *val, SANE_Int size, char *buf, unsigned buf_size )
{
	char* buf_start = buf;
...
static unsigned int
print_fixed_vec( const SANE_Fixed *val, SANE_Int size, char *buf, unsigned buf_size )
{
	char* buf_start = buf;
	unsigned int pos = 0, chars = 0;
	int chars_left = buf_size;
...
static void
OyInit_color_options(SANE_Handle device)
{
