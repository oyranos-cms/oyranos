...
int myMain( int argc, const char ** argv )
{
  ...
  oyjlOptionChoice_s A_choices[] = {{_("Put the following in a setup script like .xinitrc"),_("# select a monitor profile, load the binary blob into X and"),_("# fill the VideoCardGammaTable, if appropriate"),_("oyranos-monitor")},
  ...
  /* type,   flags,                      o,  option,          key,      name,          description,                  help, value_name,         
    ...
    {"oiwi", OYJL_OPTION_FLAG_EDITABLE,  "d","device-pos",    NULL,     NULL,          _("position in device list. The numbering of monitors starts with zero for the first device. To get the number of all available devices use \"oyranos-monitor -l | wc -l\"."),NULL, _("NUMBER"),
    ...
    {"oiwi", OYJL_OPTION_FLAG_EDITABLE,  NULL,"module",        NULL,     _("Module"),   _("select module"),          NULL, _("MODULENAME"),
    ...
    {"oiwi", OYJL_OPTION_FLAG_EDITABLE,  "f","format",        NULL,     _("Format"),   _("[edid|icc|edid_icc|vcgt] Get data"),_("Select a data format. Possible are edid for server side EDID data, icc for a server side ICC profile, edid_icc for a ICC profile created  from  server side  EDID and vcgt for effect and monitor white point merged into a possibly altered VCGT tag. Without the -o/--ouput option the output is written to stdout."),_("edid|icc|edid_icc|vcgt"),
        oyjlOPTIONTYPE_CHOICE,   {0},                oyjlSTRING,    {.s=&format}, NULL},
    {"oiwi", OYJL_OPTION_FLAG_EDITABLE,  "o","output",        NULL,     _("Output"),   _("OUT_FILENAME Write data selected by -f/--format to the given filename."),NULL, _("OUT_FILENAME|default:stdout"),
        oyjlOPTIONTYPE_CHOICE,   {0},                oyjlSTRING,    {.s=&output}, NULL},
    {"oiwi", 0,                          "m","device-meta-tag",NULL,    NULL,          _("(--device-meta-tag) switch is accepted by the -f=icc and -f=edid_icc option. It embeddes device and driver informations about the actual device for"),_("later easier ICC profile to device assignment.  This becomes useful for sharing ICC profiles."),NULL,
