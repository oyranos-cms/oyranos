...
int oy{{ class.baseName }}_Release_( {{ class.privName }} **{{ class.baseName|lower }} )
{
  ...
  *{{ class.baseName|lower }} = 0;
  ...
  if(refs < 0)
  {
    WARNc2_S( "oy{{ class.baseName }}_s[%d]->object can not be untracked with refs: %d\n", id, refs );
...
{% endblock customDestructor %}
  ...
  if(s->oy_->deallocateFunc_)
  {
    ...
    if(refs > 1)
      fprintf( stderr, "!!!ERROR:%d oy{{ class.baseName }}_s[%d]->object can not be untracked with refs: %d\n", __LINE__, id, refs);
