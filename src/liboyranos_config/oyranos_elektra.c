                                       ...
                                       int               * n )
{
  ...
  if(!error)
    my_key_set = oyReturnChildrenList_( name, scope, &rc );
                 ...
                 oyAlloc_f         allocate_func )
{
  ...
  name = oyDBGetStringFast_( oy_handle_, key_name, scope, allocate_func );
                                       ...
                                       oySCOPE_e           scope )
{
  ...
  {
    key = key_names[i];
    value = oyDBGetStringFast_( oy_handle_, key, scope, oyAllocateFunc_ );
    if(value)
    {
      if(value[0])
      {
